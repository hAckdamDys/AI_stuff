...
int main(){
	...
	const unsigned outputNeurons=4;
	topology.push_back(inputNeurons);//input
	topology.push_back(1);//hidden
	...
	while(1){
		...
		for(int i=0;i<inputNeurons;i++){
			inputVals.push_back(rand()/double(RAND_MAX));
		}
		myNet.feedForward(inputVals);
		
		targetVals.clear();
		targetVals.push_back((inputVals[0]+inputVals[1])/10);
		targetVals.push_back((inputVals[0]+inputVals[2])/10);
		targetVals.push_back((inputVals[1]+inputVals[2])/10);
		targetVals.push_back((inputVals[0]+inputVals[1]+inputVals[2])/10);
