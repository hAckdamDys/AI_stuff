...
class Neuron{
	public:
		...
		void showWeights(const Layer &nextLayer) const;
	private:
...
void Neuron::showWeights(const Layer &nextLayer) const{
	for(unsigned i=0;i<nextLayer.size()-1;++i){//-1 bo bias
		cout<<"		"<<m_outputWeights[i].weight<<endl;
	}
}
...
void Net::showWeights() const{
	for(unsigned i=0;i<m_layers.size()-1;++i){
		...
		for(unsigned j=0;j<m_layers[i].size();++j){
			cout<<"	Neuron "<<j<<endl;
			m_layers[i][j].showWeights(m_layers[i+1]);
		}
	}
}

//to do rozdzielić na pare plików z net neuron main co najmniej 3
int main(){
	srand(time(NULL));
	...
	while(1){
		...
		if(inputVals[0]>inputVals[1]){
			...
			if(inputVals[1]>inputVals[2]*inputVals[2]){
